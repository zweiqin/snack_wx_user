<template>
	<view :style="colorStyle">
		<view class="my-order">
			...
			<view class="list">
				<view v-for="(item, index) in orderList" :key="index" class="item">
					<view @click="goOrderDetails(item.order_id)">
						<view class="title acea-row row-between-wrapper">
							<view class="acea-row row-middle">
								<text
									v-if="item.bargain_id != 0"
									class="sign cart-color acea-row row-center-wrapper"
								>砍价</text>
								<text
									v-else-if="item.combination_id != 0"
									class="sign cart-color acea-row row-center-wrapper"
								>拼团</text>
								<text
									v-else-if="item.seckill_id != 0"
									class="sign cart-color acea-row row-center-wrapper"
								>秒杀</text> <text
									v-else-if="item.advance_id != 0"
									class="sign cart-color acea-row row-center-wrapper"
								>预售</text>
						...
						<view
							v-for="(item, index) in item.cartInfo" :key="index"
					...
					<view class="bottom acea-row row-right row-middle">
						...
						<view
							v-if="item._status._type == 4" class="bnt cancelBnt"
			...
			<view v-if="orderList.length > 0" class="loadingicon acea-row row-center-wrapper">
	...
	methods: {
		...
		getOrderList() {
			...
			getOrderList({
				...
				.then((res) => {
					const list = res.data || []
					const loadend = list.length < that.limit
