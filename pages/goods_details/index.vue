		...
		>
			...
			<view>
				...
				>
					<view id="past0">
							...
							>
								<view class="hide line1 acea-row">
									...
									<template v-for="(item, index) in couponList">
										<view v-if="index < 2" :key="index" class="activity">
						...
						<view>
							<uni-card title="商品过期时间">
								...
								<text class="uni-body" :style="{color:(storeInfo.expiration_time&&new Date(storeInfo.expiration_time).getTime()<new Date().getTime())?'red':''}">{{ storeInfo.expiration_time||'无' }}</text>
					...
					<!-- 优品推荐 -->
					<view v-if="good_list.length" id="past2" class="superior">
						...
						<view class="slider-banner banner">
							...
							>
								<swiper-item v-for="(item, indexw) in good_list" :key="indexw">
									<view :id="'list' + indexw" class="list acea-row row-middle">
										<view
					...
					<view id="past3" class="product-intro">
			...
			<view class="footer acea-row row-between-wrapper">
				...
				<view class="item skeleton-rect" @click="setCollect">
					<view
				...
				>
					<view class="iconfont icon-gouwuche1">
						<text v-if="parseFloat(CartCount) > 0" class="num bg-color">{{
							CartCount || 0
						}}</text>
					</view>
			...
			<view v-if="posters" class="mask" @click="listenerActionClose"></view>
			<!-- #ifdef MP -->
			<!-- <authorize @onLoadFun="onLoadFun" :isAuto="isAuto" :isShowAuth="isShowAuth" @authColse="authColse"></authorize> -->
			<!-- #endif -->
			<!-- 海报展示 -->
			<view v-if="posterImageStatus" class="poster-pop">
				<image
			...
			<view v-if="posterImageStatus" class="mask"></view>
			<canvas v-if="canvasStatus" class="canvas" canvas-id="myCanvas"></canvas>
			<!-- 发送给朋友图片 -->
			...
			<kefuIcon
				:ids="parseInt(id)"
				:routine-contact="routineContact"
...
export default {
	...
	onLoad(options) {
		...
		// #ifdef MP
		if (options.scene) {
			const value = that.$util.getUrlParams(decodeURIComponent(options.scene))
   ...
   */
	...
	methods: {
		...
		appShare(scene) {
			...
			const curRoute = routes[routes.length - 1].$page.fullPath // 获取当前页面路由，也就是最后一个打开的页面路由
			uni.share({
				provider: 'weixin',
				scene,
				type: 0,
     ...
     */
		...
		ChangCoupons(e) {
			const coupon = e
			const couponList = this.$util.ArrayRemove(this.couponList, 'id', coupon.id)
		...
		getGoodsDetails() {
			...
			getProductDetail(that.id)
				.then((res) => {
					...
					const good_list = res.data.good_list || []
					const count = Math.ceil(good_list.length / 6)
					const goodArray = new Array()
					for (let i = 0; i < count; i++) {
						const list = good_list.slice(i * 6, i * 6 + 6)
						if (list.length) {
							goodArray.push({
								list
							})
					...
					for (const key in res.data.productValue) {
						const obj = res.data.productValue[key]
     ...
     */
		...
		getCouponList(type) {
			...
			getCoupons(obj).then((res) => {
				...
				if (type === undefined || type === null) {
					const count = [ ...that.coupon.count ]
					let indexs = ''
					const index = count.findIndex((item) => item)
					const delCount = that.coupon.count
					const newDelCount = []
					let countIndex = 0
     ...
     */
		...
		goCat(news) {
			...
			} else if (that.isOpen) that.attr.cartAttr = true
     ...
     */
		...
		goPoster() {
			...
			const arr2 = [that.posterbackgd, that.storeImage, that.PromotionCode]
