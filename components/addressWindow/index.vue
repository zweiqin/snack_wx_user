<template>
	<view>
		<view class="address-window" :class="address.address==true?'on':''">
			...
			<view class="list">
				<view
					v-for="(item,index) in addressList" :key="index" class="item acea-row row-between-wrapper"
					:class="active==index?&quot;font-num&quot;:&quot;&quot;" @tap="tapAddress(index,item.id)"
				>
			...
			<!-- 无地址 -->
			<view v-if="!is_loading && !addressList.length" class="pictrue">
